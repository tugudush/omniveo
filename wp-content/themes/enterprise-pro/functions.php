...
function custom_styles_scripts() {            
    wp_enqueue_style( 'custom-style', get_stylesheet_directory_uri() . '/custom.css', array(), '0.0.1');
    wp_enqueue_style( 'custom-media', get_stylesheet_directory_uri() . '/media.css', array(), '0.0.1');            
    wp_enqueue_script('custom-js', get_stylesheet_directory_uri() . '/js/custom.js', array('jquery'), '0.0.1', true);
}

function home_banner() {
    if (is_front_page()) {
        if ( function_exists( 'soliloquy' ) ) { soliloquy( '57' ); }
    }
} // End of function home_banner()

add_action('wp_enqueue_scripts', 'custom_styles_scripts', 11);
add_action('genesis_after_header', 'home_banner', 11);